    ...
    <ul className={`rounded-t-md ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
      ...
      {(provided) => (
        <li
          ...
          className={`flex items-start p-5 border-b-2 ${
            isDarkMode ? 'border-slate-400' : 'border-slate-200'
          }`}
        >
          <div
            onClick={() => handleTaskCompletion(todo.id)}
            className={`border-2 bg-no-repeat bg-center rounded-full w-5 h-5 flex items-center justify-center ${
              isDarkMode ? 'border-gray-600' : ''
            } ${completedTasks.includes(todo.id) ? 'bg-custom-gradient' : ''}`}
